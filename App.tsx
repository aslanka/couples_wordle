  ...
  return (
    <KeyboardAvoidingView style={styles.authPage} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.authScroll} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.authSub}>{mode === 'signup' ? 'Make an account, pair once, then trade one five-letter word whenever a new Pairle opens.' : 'Your person and today’s word are right where you left them.'}</Text>
  ...
  async function refresh(quiet = false) {
    try {
      ...
      const [dashResult, statsResult, historyResult] = await Promise.all([
        ...
        supabase.rpc('get_pair_history_test'),
      ]);
  ...
  return (
    <ScrollView contentContainerStyle={styles.pageWithNav}>
      ...
      <View style={styles.historyDateRow}>
        ...
        <Pressable style={styles.historyDateCenter} onPress={() => setPickerOpen((v) => !v)}><Text style={styles.historyDate}>{formatRoundDate(selected)}</Text><Text style={styles.historyTime}>{formatRoundTime(selected)} · tap to pick</Text></Pressable>
...
function SetWord({ dashboard, onDone, onBack }: { dashboard: Dashboard; onDone: (d: Dashboard) => void; onBack: () => void }) {
  ...
  async function save() { if (word.length !== 5) return; try { setBusy(true); const { data, error } = await supabase.rpc('submit_word_test', { word }); if (error) throw error; onDone(data as Dashboard); sendPairlePush('word_sent').catch(() => undefined); Alert.alert('Sent ♡', `${dashboard.partnerName} can now play this word.`); } catch (e) { Alert.alert('Could not send', e instanceof Error ? e.message : 'Try again'); } finally { setBusy(false); } }
  return <KeyboardAvoidingView style={styles.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><Back onPress={onBack} /><CountdownBar /><Text style={styles.eyebrow}>SECRET WORD</Text><Text style={styles.hero}>Pick a word for {dashboard.partnerName}.</Text><Text style={styles.sub}>Five letters. A tiny challenge from you to them.</Text><WordTiles word={word} /><TextInput value={word} onChangeText={(v) => setWord(normalizeWord(v))} autoFocus maxLength={5} autoCapitalize="characters" autoCorrect={false} style={styles.hiddenInput} /><PrimaryButton title={busy ? 'Sending…' : 'Send this word'} onPress={save} disabled={busy || word.length !== 5} /></KeyboardAvoidingView>;
}

function CountdownBar() { const now = useClock(); const diff = Math.max(0, nextTestPairle(now).getTime() - now.getTime()); const mins = Math.floor(diff / 60000); const secs = Math.floor((diff % 60000) / 1000); return <View style={styles.countdownBar}><Text style={styles.currentTime}>{now.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</Text><View style={styles.countdownDivider} /><View style={{ flex: 1 }}><Text style={styles.countdownLabel}>NEXT PAIRLE · TEST MODE</Text><Text style={styles.countdownValue}>{mins}m {String(secs).padStart(2, '0')}s</Text></View></View>; }
...
function historyBoardStatus(board: BoardSnapshot) { if (!board) return 'No word this round'; if (board.solved) return `Solved in ${board.guesses.length}/6`; if (board.finished) return `Missed · ${board.answer ?? ''}`; return `${board.guesses.length}/6 guesses used`; }
...
function nextTestPairle(now: Date) { const slot = 2 * 60 * 1000; return new Date(Math.floor(now.getTime() / slot) * slot + slot); }
function historyDate(round?: HistoryRound) { if (!round) return new Date(); if (round.displayAt.includes('T')) return new Date(round.displayAt); const [y, m, d] = round.displayAt.split('-').map(Number); return new Date(y, (m || 1) - 1, d || 1, 12); }
...
function formatRoundTime(round?: HistoryRound) { return historyDate(round).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }); }
