    ...
    <SafeAreaView style={styles.safe}>
      ...
      {session ? (
        ...
        <AuthScreen
          ...
          onSignupComplete={() => setShowOnboarding(true)}
        />
      )}
    </SafeAreaView>
  );
...
function AuthScreen({ initialMode, onModeChange, onSignupComplete }: {
  initialMode: AuthMode;
  ...
  async function submit() {
    if (!email.trim() || password.length < 6) {
      return Alert.alert('Almost there', 'Add an email and a password with at least 6 characters.');
    }
    try {
      setBusy(true);
      if (mode === 'signup') {
  ...
  function socialPreview(provider: string) {
    Alert.alert(`${provider} sign in`, 'This will be wired up before launch. For now, use email so we can keep testing the flow.');
  }
    ...
    return (
      <KeyboardAvoidingView style={styles.authPage} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.authScroll} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.authHero}>Welcome back.</Text>
          <Text style={styles.authSub}>Your person and today’s word are right where you left them.</Text>
          ...
          <View style={styles.authFooterRow}>
            <Text style={styles.authFooterText}>New to Pairle?</Text>
            <Pressable onPress={() => switchMode('signup')}><Text style={styles.authFooterLink}> Create account</Text></Pressable>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
    );
  ...
  return (
    <KeyboardAvoidingView style={styles.authPage} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.authScroll} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.authHero}>A tiny daily thing, just for you two.</Text>
        <Text style={styles.authSub}>Make an account, pair once, then trade one five-letter word whenever a new Pairle opens.</Text>

        <View style={styles.authCard}>
          <Input label="YOUR NAME" value={name} onChangeText={setName} placeholder="Your name" />
          <Input label="EMAIL" value={email} onChangeText={setEmail} placeholder="you@email.com" autoCapitalize="none" keyboardType="email-address" />
          <Input label="PASSWORD" value={password} onChangeText={setPassword} placeholder="••••••••" secureTextEntry />
          <PrimaryButton title={busy ? 'Creating…' : 'Create account'} onPress={submit} disabled={busy} />
        </View>
        ...
        <View style={styles.authFooterRow}>
          ...
          <Pressable onPress={() => switchMode('signin')}><Text style={styles.authFooterLink}> Sign in</Text></Pressable>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
function OnboardingScreen({ onDone }: { onDone: () => void }) {
  ...
  const slides = [
    ...
    { kicker: 'SEE WHO GETS IT', big: 'Six tries. No peeking.', body: 'Solve their word, compare your guess averages, and come back when the next Pairle opens.', art: '6' },
  ];
  ...
  return (
    <View style={styles.onboardingPage}>
      ...
      <View style={styles.onboardingArt}>
        <Text style={[styles.onboardingArtText, slide.art === 'WORD' && styles.onboardingWordArt]}>{slide.art}</Text>
      </View>
      <Text style={styles.eyebrow}>{slide.kicker}</Text>
      <Text style={styles.onboardingHero}>{slide.big}</Text>
      <Text style={styles.onboardingBody}>{slide.body}</Text>
      <View style={styles.onboardingBottom}>
        <View style={styles.dotsRow}>
          {slides.map((_, i) => <View key={i} style={[styles.onboardingDot, i === step && styles.onboardingDotActive]} />)}
        </View>
        <Pressable style={styles.nextCircle} onPress={() => isLast ? onDone() : setStep((s) => s + 1)}>
  ...
  async function refresh(quiet = false) {
    try {
      ...
      const [{ data, error }, { data: statData, error: statError }] = await Promise.all([
        supabase.rpc('get_pair_dashboard_test'),
        supabase.rpc('get_pair_stats_test'),
      ]);
      if (error) throw error;
...
function PairSetup({ onDone }: { onDone: () => void }) {
  ...
  return (
    <View style={styles.page}>
      ...
      {mode === 'join' && <View style={styles.card}><Input label="INVITE CODE" value={code} onChangeText={(v: string) => setCode(v.toUpperCase().slice(0, 6))} placeholder="ABC123" /></View>}
    ...
    <ScrollView contentContainerStyle={styles.pageWithNav}>
      ...
      {waitingForPartner ? (
        <View style={styles.heroCard}>
          ...
          <Text style={styles.subCompact}>Once they join, this becomes your shared Pairle.</Text>
          <SecondaryButton title="Refresh" onPress={onRefresh} />
        </View>
      ) : (
        <>
          <View style={styles.heroCard}>
            <Text style={styles.eyebrow}>CURRENT PAIRLE</Text>
            <Text style={styles.cardHero}>{dashboard.completed ? 'You both finished.' : `You + ${dashboard.partnerName}`}</Text>
            <Text style={styles.subCompact}>{dashboard.completed ? 'This little round is done. A fresh one opens when the timer hits zero.' : 'A tiny challenge, made for each other.'}</Text>
          ...
          <View style={styles.statsCard}>
            <View style={styles.statsHeaderRow}>
              ...
              <Text style={styles.statsMini}>{stats?.completedGames ?? 0} rounds</Text>
            </View>
          ...
          <ActionCard title={`For ${dashboard.partnerName}`} status={dashboard.myWordReady ? 'Word locked in' : 'Pick this round’s word'} detail={dashboard.myWordReady ? 'They’ll get it whenever they’re ready.' : 'Choose a five-letter word just for them.'} button={!dashboard.myWordReady ? 'Create word' : undefined} onPress={onSetWord} />
          <View style={[styles.actionCard, styles.lavenderCard]}>
            ...
            <Text style={styles.actionDetail}>{dashboard.myResult?.finished ? 'Your board stays saved in the Wordle tab until the timer resets.' : dashboard.partnerWordReady ? 'Open Wordle whenever you want to play.' : 'You’ll get a notification when their word is ready.'}</Text>
          </View>
        </>
      )}
...
function WordleScreen({ dashboard, onDone }: { dashboard: Dashboard; onDone: (d: Dashboard) => void }) {
  ...
  async function submit() {
    ...
    try {
      setBusy(true);
      const { data, error } = await supabase.rpc('submit_guess_test', { attempt: draft });
      if (error) throw error;
      setDraft('');
      ...
      {!dashboard.partnerWordReady ? (
        ...
        <>
          <View style={styles.wordleTopCard}>
            <Text style={styles.eyebrow}>THIS ROUND</Text>
            ...
            {dashboard.myResult?.finished && <Text style={styles.wordleSub}>This board stays here until the countdown resets.</Text>}
          </View>
  ...
  return (
    <ScrollView contentContainerStyle={styles.pageWithNav}>
      ...
      <View style={styles.pairedCard}>
        ...
        <Text style={styles.pairedSub}>Paired quietly · just the two of you</Text>
      </View>
      <View style={styles.accountStatsRow}>
        <MiniStat label="Rounds" value={String(stats?.completedGames ?? 0)} />
...
function SetWord({ dashboard, onDone, onBack }: { dashboard: Dashboard; onDone: (d: Dashboard) => void; onBack: () => void }) {
  ...
  async function save() {
    ...
    try {
      setBusy(true);
      const { data, error } = await supabase.rpc('submit_word_test', { word });
      if (error) throw error;
      onDone(data as Dashboard);
      sendPairlePush('word_sent').catch(() => undefined);
      Alert.alert('Sent ♡', `${dashboard.partnerName} can now play this word.`);
    } catch (e) {
  ...
  return (
    <KeyboardAvoidingView style={styles.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <PrimaryButton title={busy ? 'Sending…' : 'Send this word'} onPress={save} disabled={busy || word.length !== 5} />
    </KeyboardAvoidingView>
  );
}
...
function CountdownBar() {
  ...
  return (
    <View style={styles.countdownBar}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={styles.countdownLabel}>NEXT PAIRLE · TEST MODE</Text>
        <Text style={styles.countdownValue}>{mins}m {String(secs).padStart(2, '0')}s</Text>
      </View>
    </View>
  );
}
...
function StatPerson({ name, average, wins }: { name: string; average?: number | null; wins: number }) {
  return (
    <View style={styles.statPerson}>
      ...
      <Text style={styles.statWins}>{wins} faster rounds</Text>
    </View>
  );
}
...
function nextTestPairle(now: Date) { const ms = now.getTime(); const slot = 2 * 60 * 1000; return new Date(Math.floor(ms / slot) * slot + slot); }

const C = {
  bg: '#FFF9F7', paper: '#FFFFFF', rose: '#D88C9A', berry: '#A85F72', lavender: '#C9B8E8', lavenderSoft: '#F3EEFB', peach: '#F4B8A6', text: '#2E2A2B', muted: '#8C7F82', border: '#EFD8DC', roseSoft: '#FDE8EA', sage: '#8FB996', grayTile: '#D9D2D3',
};
...
const styles = StyleSheet.create({
  ...
  authPage: { flex: 1, backgroundColor: C.bg }, authScroll: { flexGrow: 1, paddingHorizontal: 24, paddingTop: 26, paddingBottom: 36 }, authBrandRow: { marginBottom: 32 }, authLogo: { fontSize: 31, fontWeight: '900', letterSpacing: -1.3, color: C.text }, authHero: { fontSize: 39, lineHeight: 43, fontWeight: '900', letterSpacing: -1.5, color: C.text, maxWidth: 350 }, authSub: { fontSize: 16, lineHeight: 24, color: C.muted, marginTop: 12, marginBottom: 8, maxWidth: 360 }, authCard: { backgroundColor: C.paper, borderRadius: 28, padding: 20, marginTop: 18, borderWidth: 1.5, borderColor: C.border }, orText: { textAlign: 'center', color: '#B09CA0', fontWeight: '800', marginVertical: 15, fontSize: 12 }, socialButton: { minHeight: 56, borderRadius: 18, borderWidth: 1.5, borderColor: C.border, backgroundColor: C.paper, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 18, marginBottom: 10 }, socialMark: { width: 24, fontSize: 18, fontWeight: '900', color: C.text, textAlign: 'center' }, socialButtonText: { fontSize: 15, fontWeight: '900', color: C.text }, authFooterRow: { flexDirection: 'row', justifyContent: 'center', marginTop: 20 }, authFooterText: { color: C.muted, fontSize: 14, fontWeight: '700' }, authFooterLink: { color: C.berry, fontSize: 14, fontWeight: '900' },
  onboardingPage: { flex: 1, paddingHorizontal: 26, paddingTop: 24, paddingBottom: 28, backgroundColor: C.bg }, onboardingTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, stepCount: { color: C.muted, fontWeight: '900', fontSize: 12 }, onboardingArt: { height: 235, marginTop: 34, marginBottom: 30, borderRadius: 38, backgroundColor: C.roseSoft, borderWidth: 1.5, borderColor: '#F2C8CE', alignItems: 'center', justifyContent: 'center' }, onboardingArtText: { fontSize: 104, lineHeight: 116, fontWeight: '900', color: C.berry, letterSpacing: -5 }, onboardingWordArt: { fontSize: 54, letterSpacing: 7 }, onboardingHero: { fontSize: 42, lineHeight: 45, fontWeight: '900', color: C.text, letterSpacing: -1.7, marginTop: 9 }, onboardingBody: { fontSize: 17, lineHeight: 26, color: C.muted, marginTop: 13, maxWidth: 360 }, onboardingBottom: { marginTop: 'auto', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, dotsRow: { flexDirection: 'row', gap: 7 }, onboardingDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#E6D7DA' }, onboardingDotActive: { width: 24, backgroundColor: C.berry }, nextCircle: { width: 62, height: 62, borderRadius: 31, alignItems: 'center', justifyContent: 'center', backgroundColor: C.berry }, nextArrow: { color: '#fff', fontWeight: '800', fontSize: 27, marginTop: -2 },
  brandPill: { alignSelf: 'flex-start', backgroundColor: C.berry, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 7, marginBottom: 20 }, brandPillText: { color: '#fff', fontWeight: '900', letterSpacing: 2, fontSize: 12 }, logo: { fontSize: 31, fontWeight: '900', letterSpacing: -1.3, color: C.text }, logoHeart: { color: C.berry }, hero: { fontSize: 38, lineHeight: 42, fontWeight: '900', letterSpacing: -1.5, color: C.text, marginTop: 8 }, sectionTitle: { fontSize: 32, fontWeight: '900', color: C.text, letterSpacing: -1.1 }, sub: { fontSize: 16, lineHeight: 24, color: C.muted, marginTop: 12, marginBottom: 22 }, subCompact: { fontSize: 16, lineHeight: 24, color: '#7D686D', marginTop: 12 }, muted: { fontSize: 14, color: C.muted, marginTop: 3 }, eyebrow: { fontSize: 12, fontWeight: '900', letterSpacing: 1.5, color: C.berry },
  card: { backgroundColor: C.paper, borderRadius: 24, padding: 18, marginVertical: 20, borderWidth: 1, borderColor: C.border }, label: { fontSize: 11, fontWeight: '900', letterSpacing: 1.2, color: C.berry }, input: { marginTop: 7, fontSize: 17, fontWeight: '700', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: C.border, color: C.text }, segment: { flexDirection: 'row', backgroundColor: '#F5E9EB', padding: 4, borderRadius: 16, marginTop: 20 }, segmentButton: { flex: 1, padding: 12, alignItems: 'center', borderRadius: 12 }, segmentActive: { backgroundColor: C.paper }, segmentText: { fontWeight: '800', color: C.muted }, segmentTextActive: { color: C.berry },
