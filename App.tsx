...
function WordTiles({ word }: { word: string }) { return <View style={styles.wordEntry}>{Array.from({ length: 5 }).map((_, i) => <View key={i} style={[styles.bigTile, word[i] && styles.bigTileFilled]}><Text style={styles.bigTileText}> {displayTileLetter(word[i])}</Text></View>)}</View>; }
